        ...
        <View style={styles.suggestedGrid}>
          {SUGGESTED_EXERCISES.map(s => (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/exercises/add?name=${encodeURIComponent(s.name)}&icon=${encodeURIComponent(s.icon)}`)}
              activeOpacity={0.7}
            >
...
const styles = StyleSheet.create({
  ...
  exCard: {
    ...
    padding: 16, marginBottom: 8, borderRadius: 14, borderWidth: 1,
  },
  exIconBg: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  exIconText: { fontSize: 22 },
  exInfo: { flex: 1 },
  exName: { fontSize: 16, fontWeight: '600' },
  exMeta: { fontSize: 12, marginTop: 3 },
  chevBtn: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  suggestedGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  suggestPill: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    paddingVertical: 9, paddingHorizontal: 12, borderRadius: 20, borderWidth: 1,
  },
